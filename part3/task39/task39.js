...
SortTable.prototype = {
	...
	addArrow: function() {
		...
		addEvent(document, "scroll", function(event) {
			console.log(self.table.offsetHeight);
			if (document.body.scrollTop >= self.table.offsetHeight) {
				self.table.children[0].children[0].style.position = "relative";			
		}
			else {
				self.table.children[0].children[0].style.position = "fixed";	
			}
		});
